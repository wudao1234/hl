(function() {
	...
	var strings = $.map(numbers, function(num){ return this.accounting.formatMoney(num*1000, "HK$ "); });
	...
	JSLitmus.test('formatNumber()', function(count) {
		var i = 0;
		while ( count-- ) {
			this.accounting.formatMoney(numbers[i]);
			i++;
			i > numbers.length && (i = 0);
		}
	});
	...
	JSLitmus.test('formatNumber(array)', function(count) {
		var i = 0;
		while ( count-- ) {
            this.accounting.formatMoney([numbers[i], numbers[i+1]]);
			i += 2;
			i > numbers.length && (i = 0);
        }
	});
