	...
	<view>
		<form>
			...
			<view class="cu-form-group">
				...
				<input placeholder="不填写则自动递增" v-model="autoIncreaseSn" />
			</view>
			<view class="cu-form-group">
			...
			<uni-list>
				<uni-swipe-action>
					<uni-swipe-action-item :right-options="options" v-for="item in orderItems" :key="item.key" @click="bindClickOrderItem(item.key)">
						<uni-list-item
							:show-arrow="false"
							:title="formatTitle(item.name, item.quantityInitial)"
							:note="formatNote(item.sn, item.price, item.quantityInitial)"
							:show-badge="true"
							:badge-text="formatPrice(item.price, item.quantityInitial)"
						/>
					</uni-swipe-action-item>
				</uni-swipe-action>
				<uni-swipe-action>
					<uni-swipe-action-item :right-options="options" v-for="stock in orderStocks" :key="stock.key" @click="bindClickOrderStock(stock.key)">
						<uni-list-item
							:show-arrow="false"
							:title="formatTitle(stock.name, stock.quantityInitial)"
							:note="formatNote(stock.sn, stock.price, stock.quantityInitial)"
							:show-badge="true"
							:badge-text="formatPrice(stock.price, stock.quantityInitial)"
						/>
					</uni-swipe-action-item>
				</uni-swipe-action>
			</uni-list>
			<view class="price_total">
				订单总金额：
				<uni-tag class="price_tag" :circle="true" :text="totalPriceInitial" type="primary" size="small" />
			</view>
			<view class="padding flex flex-direction"><button class="cu-btn bg-blue margin-tb-sm lg" type="primary" :loading="loading" @click="saveOrder">保存订单</button></view>
		</form>
	</view>
</template>

<script>
import { uniList, uniListItem, uniTag } from '@dcloudio/uni-ui';
...
export default {
	components: { uniList, uniListItem, uniTag },
	data() {
		return {
			options: [
				{
	...
	computed: {
		...
		totalPriceInitial() {
			const orderItemsPrice = this.orderItems
				.map(item => {
					return item.price * item.quantityInitial;
				})
				.reduce((sum, totalPrice) => sum + totalPrice, 0);
			const orderStocksPrice = this.orderStocks
				.map(item => {
					return item.price * item.quantityInitial;
				})
				.reduce((sum, totalPrice) => sum + totalPrice, 0);
	...
	methods: {
		...
		saveOrder() {
			...
			this.loading = true;
			this.api
				.post('/api/customer_orders?useNewAutoIncreaseSn=' + this.resetSn + '&fetchStocks=' + this.fetchStocks, {
	...
	onLoad() {
		...
		uni.$on('orderItemsForOrder', valueOrderItems => {
			this.middleOrderItems = [...this.orderItems].concat(valueOrderItems);
		});
		uni.$on('orderStocksForOrder', valueOrderStocks => {
			this.middleOrderStocks = [...this.orderStocks].concat(valueOrderStocks);
		});
	},
	onUnload() {
	...
	onShow() {
		this.orderItems = [...this.middleOrderItems]
		this.orderStocks = [...this.middleOrderStocks]
	}
};
</script>
