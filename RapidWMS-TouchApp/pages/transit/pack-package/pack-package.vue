<template>
	<view>
		...
		<uni-list>
			<uni-list-item :show-arrow="false" :title="'总件数：' + packages" />	
		</uni-list>
		<view class="title">件码分配</view>
		<uni-list>
			<block v-for="item in packItems" :key="item.id">
				<uni-swipe-action :options="options" @click="bindClickPackItem(item)">
					<uni-list-item :key="item.id" :show-arrow="false"
		...
		<uni-list>
			<uni-list-item v-for="stock in stockFlows" :key="stock.id" :show-arrow="true" 
				:title="formatTitle2(stock.name, stock.quantity)" 
				:note="formatGoodsNote(stock.sn, stock.expireDate)" @click="addItem(stock)"
				:show-badge="true" :badge-text="stock.quantity" badge-type="error" />	
		</uni-list>
		<view class="price_total" v-if="stockFlows.length == 0">
			已分配完毕
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			packagePack() {
				this.loading = true;
				this.api.post('/api/packs/packages', {
					id: this.packId,
