<template>
	<view>
		...
		<view class="padding flex flex-direction">
			...
			<button class="cu-btn bg-cyan margin-tb-sm lg" :loading="loading" @click="reUploadPhoto" v-if="packStatus == 'CLIENT_SIGNED' || packStatus == 'COMPLETE'">
				重传照片
			</button>
		...
		<view class="cu-modal" :class="showModal ? 'show' : ''">
			<view class="cu-dialog">
				...
				<view>
					<view class="cu-form-group"><input v-model="cancelDescription" class="input_area" placeholder="必须填入原因" maxlength="120" /></view>
				</view>
				<view class="cu-bar bg-white justify-end">
					<view class="action">
	...
	methods: {
		...
		confirmCancel() {
			if (this.cancelDescription.length > 0) {
				...
				this.loading = true;
				this.api
					.post('/api/packs/cancel/' + this.packId + '?description=' + this.cancelDescription)
					.then(res => {
						if (res.statusCode == 201) {
							uni.showToast({
								title: '作废成功',
