<template>
	<view>
		...
		<uni-list>
			<uni-list-item v-for="item in stockFlowItems" :key="item.id" :show-arrow="false" 
				:title="formatTitle(item.name, item.quantity)" 
				:note="formatNote(item.sn, item.warePositionOut, item.expireDate)"
				:show-badge="item.confirm == true" :badge-type="formatBadgeType(item)" :badge-text="formatBadgeText(item)" />
		</uni-list>
		<view class="price_total">
			总金额：<uni-tag class="price_tag" :circle="true" :text="totalPrice" type="error" size="small" />
		</view>
		...
		<view class="padding flex flex-direction" v-if="orderStatus == 'GATHERING_GOODS'">
			...
			<button class="cu-btn bg-red margin-tb-sm lg" type="primary" :loading="loading" :disabled="notConfirm()" @click="confirmWithSacn">{{this.confirmButtonText()}}</button>
	...
	export default {
		...
		methods: {
			...
			gatheringGoods() {
				...
				this.loading = true;
				this.api.post("/api/customer_orders/gather_goods/" + this.orderId).then(res => {
