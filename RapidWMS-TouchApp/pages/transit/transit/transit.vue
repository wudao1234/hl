...
<script>
	...
	export default {
		...
		methods: {
			...
			loadOrders() {
				this.loading = true;
				this.api.get("/api/customer_orders/list_for_gather_confirm?size=" + this.pageCount + "&page=" + this.currentPage + (this.searchValue ? "&search=" + this.searchValue : "") + this.getFilterQueryString()).then(res => {
			...
			loadPacks() {
				this.loading2 = true;
				this.api.get("/api/packs?size=" + this.pageCount2 + "&page=" + this.currentPage2 + (this.searchValue2 ? "&search=" + this.searchValue2 : "") + this.getFilterQueryString2()).then(res => {
					if (res.statusCode == 200) {
			...
			loadAddress() {
				this.loading3 = true;
				this.api.get("/api/address?size=" + this.pageCount3 + "&page=" + this.currentPage3 + (this.searchValue3 ? "&search=" + this.searchValue3 : "")).then(res => {
					if (res.statusCode == 200) {
			...
			loadSendingPacksGroupByUsers() {
				this.loading4 = true;
				this.api.get("/api/packs/list_sending_packs_group_by_users").then(res => {
					if (res.statusCode == 200) {
