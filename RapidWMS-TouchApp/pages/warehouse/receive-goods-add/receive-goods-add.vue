	...
	<view>
		<form>
			...
			<view class="cu-form-group">
				...
				<input class="text-right" placeholder="选填" v-model="description" />
			</view>
			<view class="button_area"><button class="cu-btn bg-green add_item" @click="chooseFromGoods">从商品选择</button></view>
			...
			<uni-list>
				<uni-swipe-action>
					<uni-swipe-action-item :right-options="options" v-for="item in items" :key="item.key" @click="bindClickItem(item.key)">
						<uni-list-item
							:show-arrow="false"
...
export default {
	...
	onLoad(params) {
		...
		uni.$on('add-receive-goods-items', item => {
			this.middleItems = [...this.items].concat(item);
		});
	},
	onUnload() {
