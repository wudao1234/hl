<template>
	<view>
		...
		<view class="price_total">
			单品数量：
			<uni-tag class="price_tag" :circle="true" :text="itemTypes()" type="error" size="small" />
			商品总数：
			<uni-tag class="price_tag" :circle="true" :text="itemAmounts()" type="error" size="small" />
		</view>
		<uni-list>
			<uni-list-item
				v-for="item in receiveGoods.receiveGoodsItems"
				:key="item.id"
				:show-arrow="false"
				:title="formatTitle(item.goods.name, item.quantityInitial)"
				:note="formatNote(item.goods.sn, item.warePosition.name, item.price)"
				:show-badge="true"
				:badge-text="formatAuditNumber(item.quantity)"
			/>
		</uni-list>
		<view class="padding flex flex-direction" v-if="canAudit">
			<button class="cu-btn bg-blue lg" :loading="loading" v-if="canUnload" @click="putInStorage">入库</button>
		...
		<view class="padding flex flex-direction" v-else><button class="cu-btn bg-red lg" :loading="loading" @click="cancelAudit">取消审核</button></view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		putInStorage() {
			this.loading = true;
			this.api
				.get(`/api/receive_goods/put_in_storage/${this.receiveGoods.id}`)
				.then(res => {
					uni.showToast({
						title: '入库成功',
						icon: 'success',
			...
			uni.showModal({
				...
				success: res => {
					if (res.confirm) {
						this.loading = true;
						this.api
							.put('/api/receive_goods/cancel_audit', {
